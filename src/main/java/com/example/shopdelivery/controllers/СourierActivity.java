...
public class СourierActivity implements Initializable {
...
//    }
    ...
    void onRun(MouseEvent event) {
        ...
        while (sc.hasNextLine()){
            ...
            switch (s.substring(0,1)) {
                case "<":
                    int index = -1;
                    String[] p =s.substring("< Изменить статус (".length(), s.length() - 1).split(",");
                    String idi = p[0];
                    for (int i = 0; i < deliveries.size(); i++) {
                        if (idi.equals(deliveries.get(i).getId())){
                            index = i;
                        }
                    }
                    ...
                    editTextD.setText("");
                    String r = "";
                    String url = "http://localhost:5050/setStatus?idDelivery=" + D + "&status=" + p[0];
                    try {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        var url = "http://localhost:5050/getDelivery?name=" + name;
        String ro = "";
        try {
            var myurl = new URL(url);
            con = (HttpURLConnection) myurl.openConnection();
        ...
        D = ro.trim(); // Номер доставки
        if (D.equals("") || D.equals("false")) {
            editTextD.setText("Нет назначенных доставок");
        }
        else {
            nameDelivery.setText(D);
            ...
            for (int i = 0; i < deliveries.size(); i++) {
                if (deliveries.get(i).getId().equals(D.trim())) {
                    index = i;
                }
            }
            Delivery one = deliveries.get(index);
            ...
            for (String p: one.getOrderList()) {
                for (Order g: orders) {
                    if (g.getId().equals(p)) {
                        listViewD.getItems().add(g.toString());
                    }
                }
            }
        }
